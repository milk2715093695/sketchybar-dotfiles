...
static inline void cpu_update(struct cpu* cpu) {
    ...
    if (cpu->has_prev_load) {
        ...
        cpu->user_load  = (double)delta_user / (double)(delta_user + delta_system + delta_idle) * 100.0;
        cpu->sys_load   = (double)delta_system / (double)(delta_user + delta_system + delta_idle) * 100.0;
